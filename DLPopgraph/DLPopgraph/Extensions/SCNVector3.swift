...
extension SCNVector3 {
    ...
    public func length() -> DLFloat {
        return DLFloat( sqrt( self.x*self.x + self.y*self.y + self.z*self.z ) )
    }
    ...
    static func lineEulerAngles(vector: SCNVector3) -> SCNVector3 {
        ...
        let pitch = vector.z == DLFloat(0.0) ? CGFloat(pitchB) : CGFloat(sign(Float(vector.z)) * pitchB)
        var yaw: Float = 0.0
        if vector.x != 0.0 || vector.z != 0.0 {
            let inner = vector.x / (height * DLFloat(sinf(Float(pitch))))
            if inner > 1 || inner < -1 {
                yaw = Float.pi / 2.0
            } else {
        ...
        return SCNVector3(DLFloat(pitch), DLFloat(yaw), 0)
    }
    ...
    static func /(lhs: SCNVector3, rhs: DLFloat ) -> SCNVector3 {
